        ...
        const int info)
{
    if ((info >> 16) == 0)
    {
        scoped_ptr<Operation> fetcher(FetchTCP(MIC0_TCP_ADDRESS,
                    LISTEN_PORT,
                    0,
                    1,
                    (1 << 16) | (info & 0xffff)));
    ...
    if ((info >> 16) == 2)
    {
        CompoundSingleSourceProjector* lineitem_projector =
...
Operation* supersonic::distribute_operation(const int thread_id,
        ...
        const int info)
{
    scoped_ptr<Operation> lineitem_fetcher(FetchTCP(CPU_TCP_ADDRESS,
                LISTEN_PORT,
                thread_id,
                thread_count,
                (2 << 16)));

    scoped_ptr<Operation> orders_fetcher(FetchTCP(CPU_TCP_ADDRESS,
                LISTEN_PORT,
                thread_id,
                thread_count,
                (3 << 16)));
    ...
    scoped_ptr<Operation> merge_batcher(MergeBatch(2, hash_joiner.release()));
