...
// another dimension tables for Q4_1 with different selectivity
...
int main(int argc, char** argv) {
	if(argc != 4) {
		std::cout << "./this q4_1_run_time q4_2_run_time memory_width" << std::endl;
		exit(-1);
	}

	int q4_1_run_time = atoi(argv[1]);
	int q4_2_run_time = atoi(argv[2]);
	int memory_width = atoi(argv[3]);
	...
	query_proportion.push_back(q4_1_run_time * 1.0 / (q4_1_run_time + q4_2_run_time));
	query_proportion.push_back(q4_2_run_time * 1.0 / (q4_1_run_time + q4_2_run_time));
	
	std::cout << "query_proportion " << query_proportion[0] << " " << query_proportion[1] << std::endl;
	
	vector<vector<double>> selectivity;
	// cuskey partkey suppkey orderdate
	vector<double> element1 = {0.4, 0.5, 0.5, 1.0};
	vector<double> element2 = {1.0, 0.5, 0.5, 0.4};
	...
	vector<vector<int>> classic_filter_order_Q4_1(1000, {0, 1, 2, 3});
	vector<vector<int>> classic_filter_order_Q4_2(1000, {3, 2, 1, 0});

	const View& classic_init_fact_view = LineorderView(init_storage_map);
	
	//int q4_run_time = q4_1_run_time; //1;
	//int q4_run_time = 4;

	// classic initial 
	clock_gettime(CLOCK_REALTIME, &begin);
	for(int i = 0; i < q4_1_run_time; i++) {
		...
		std::cout<<"q4_1 begin caculate"<<std::endl;

		ResultView resv(ResultView(cr->Next(-1)));
		View rv(resv.view());
		std::cout << "q4_1 result" << std::endl;
		// ViewPrinter view_printer;
		// view_printer.AppendViewToStream(rv, &cout);
	}

	for(int i = 0; i < q4_2_run_time; i++) {
		vector<Operation*> lhs_children;
		...
		std::cout<<"q4_2 begin caculate"<<std::endl;

		ResultView resv(ResultView(cr->Next(-1)));
		View rv(resv.view());
		std::cout << "q4_2 result" << std::endl;
		// ViewPrinter view_printer;
		// view_printer.AppendViewToStream(rv, &cout);
	}
	...
	for(int i = 0; i < q4_1_run_time; i++) {
		...
		std::cout<<"classic q4_1 begin caculate"<<std::endl;

		ResultView resv(ResultView(cr->Next(-1)));
		View rv(resv.view());
		std::cout << "classic q4_1 result" << std::endl;
		// ViewPrinter view_printer;
		// view_printer.AppendViewToStream(rv, &cout);
	}
	...
	vector<vector<int>> optimized_filter_order_Q4_1 = *optimizer->optimized_filter_order(0);
	vector<vector<int>> optimized_filter_order_Q4_2 = *optimizer->optimized_filter_order(1);
	clock_gettime(CLOCK_REALTIME, &end);
...
*/
	...
	for(int i = 0; i < q4_1_run_time; i++) {
		...
		std::cout<<"optimized q4_1 begin caculate"<<std::endl;

		ResultView resv(ResultView(cr->Next(-1)));
		View rv(resv.view());
		std::cout << "optimized q4_1 result" << std::endl;
		// ViewPrinter view_printer;
		// view_printer.AppendViewToStream(rv, &cout);
	}
	...
	std::cout << "Result"
	<< " q4_1_run_time " << q4_1_run_time
	<< " q4_2_run_time " << q4_2_run_time
