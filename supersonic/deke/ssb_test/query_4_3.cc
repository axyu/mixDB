...
#include "deke/include/ssb_q4_3.h"
...
const View& LineorderView(vector<vector<bool>>& storage_map) {
	...
	TupleSchema lineorder_schema = SSB_Q4_3_Schema::lineorder();	
	File* fp = File::OpenOrDie("../../storage/lineorder_q4_3.tbl", "r");
...
const View* CustomerView() {
	...
	TupleSchema customer_schema = SSB_Q4_3_Schema::customer();
	File* fp = File::OpenOrDie("../../storage/customer_q4_3.tbl", "r");
...
const View* SupplierView() {
	...
	TupleSchema supplier_schema = SSB_Q4_3_Schema::supplier();
	File* fp = File::OpenOrDie("../../storage/supplier_q4_3.tbl", "r");
...
const View* PartView() {
	...
	TupleSchema date_schema = SSB_Q4_3_Schema::part();
	File* fp = File::OpenOrDie("../../storage/part_q4_3.tbl", "r");
...
const View* DateView() {
	...
	TupleSchema date_schema = SSB_Q4_3_Schema::date();
	File* fp = File::OpenOrDie("../../storage/date_q4_3.tbl", "r");
	FailureOrOwned<Cursor> cu(FileInput(date_schema, fp, false, HeapBufferAllocator::Get()));
	Table* table = new Table(date_schema, HeapBufferAllocator::Get());
	vector<StorageType> storage_type(1, MEMORY);
	vector<rowcount_t> in_memory_row_capacity_vector(1, date_row_count);
...
int main() {
	...
	vector<double> element = {0.2, 0.2, 0.4, 6.0 / 7.0};
	selectivity.push_back(element);

	TupleSchema fact_schema = SSB_Q4_3_Schema::lineorder();
	vector<TupleSchema> dimension_schemas;
	TupleSchema dimension1_schema = SSB_Q4_3_Schema::customer();
	TupleSchema dimension2_schema = SSB_Q4_3_Schema::supplier();
	TupleSchema dimension3_schema = SSB_Q4_3_Schema::part();
	TupleSchema dimension4_schema = SSB_Q4_3_Schema::date();
	...
	dimension_views.push_back(PartView());
	...
	vector<vector<int>> classic_filter_order(1000, {0, 2, 1, 3});
