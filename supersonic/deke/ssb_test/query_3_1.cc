...
const View& LineorderView(vector<vector<bool>>& storage_map) {
	...
	for(int map_row_id = 0; map_row_id < storage_map.size(); map_row_id++) {
		in_memory_row_capacity_vector[12] += rowGroupSize;
		storage_type_vector[map_row_id][12] = MEMORY;
...
int main() {
	...
	vector<vector<bool>> init_storage_map(row_group_count, vector<bool>(key_column_count, true));
