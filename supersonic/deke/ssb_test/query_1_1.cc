...
#include "deke/include/ssb_q1_1.h"
...
const View& LineorderView(vector<vector<bool>>& storage_map) {
	...
	TupleSchema lineorder_schema = SSB_Q1_1_Schema::lineorder();	
	File* fp = File::OpenOrDie("../../storage/lineorder_q1_1.tbl", "r");
...
const View* GroupKeyView() {
	int supplier_row_count = 6001215;
	TupleSchema supplier_schema = SSB_Q1_1_Schema::groupkey();
	File* fp = File::OpenOrDie("../../storage/lineorder_q1_1.tbl.key", "r");
...
const View* DateView() {
	...
	TupleSchema date_schema = SSB_Q1_1_Schema::date();
	File* fp = File::OpenOrDie("../../storage/date_q1_1.tbl", "r");
...
int main() {
	...
	int query_count = 1;
	int fact_column_count = 19;
	int key_column_count = 2;
	vector<int> column_width = {8, 8, 8, 8, 8, 8, 128, 128, 8, 8, 8, 8, 8, 8, 8, 8, 128, 8, 8};
	vector<double> query_proportion = {1};
	vector<vector<double>> selectivity;
	vector<double> element = {0.02, 1.0};
	selectivity.push_back(element);

	TupleSchema fact_schema = SSB_Q1_1_Schema::lineorder();
	vector<TupleSchema> dimension_schemas;
	TupleSchema dimension1_schema = SSB_Q1_1_Schema::groupkey();
	TupleSchema dimension2_schema = SSB_Q1_1_Schema::date();
	dimension_schemas.push_back(dimension1_schema);
	dimension_schemas.push_back(dimension2_schema);

	vector<vector<double>> init_visit_locality(2, vector<double>(1000, 0.001));

	int memory_limit = rowGroupSize * 24 * row_group_count;

	vector<int> key_column_width = {8, 8};
	...
	dimension_views.push_back(GroupKeyView());
	...
	vector<vector<int>> classic_filter_order(1000, {1, 0});
	...
	ag_gr_p->add(ProjectNamedAttribute("LO_AGGKEY"));
