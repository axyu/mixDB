...
int main(int argc, char** argv) {
	...
	vector<double> element1 = {-1, 0.2, 0.5, 0.5};
	vector<double> element2 = {0.4, -1, 0.5, 0.4};
	vector<double> element3 = {0.4, 1.0, 0.4, 0.4};
	...
	vector<vector<int>> classic_filter_order_Q3_1(1000, {0, 2, 1});
	vector<vector<int>> classic_filter_order_Q4_1(1000, {0, 2, 3, 1});
	...
	for(int i = 0; i < q3_run_time; i++) {
		...
		std::cout<<"q3_1 begin caculate"<<std::endl;

		ResultView resv(ResultView(cr->Next(-1)));
		View rv(resv.view());
		std::cout << "q3_1 result" << std::endl;
		ViewPrinter view_printer;
		view_printer.AppendViewToStream(rv, &cout);
	}
	...
	vector<vector<int>> optimized_filter_order_Q4_1 = *optimizer->optimized_filter_order(2);
