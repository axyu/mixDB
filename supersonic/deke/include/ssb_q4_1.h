#ifndef INCLUDE_SSB_Q4_1_H
#define INCLUDE_SSB_Q4_1_H
...
public:
	...
	static TupleSchema part() {
		TupleSchema supplier_schema;
		supplier_schema.add_attribute(Attribute("P_MFGR", STRING, NULLABLE));
		return supplier_schema;
	}
...
public:
	static void Customer(std::string tbl_path, std::string binary_path){
		...
		while(fgets(buffer, 8192, fp)) {
			...
			for(int i = 0; i < 8 && p; i++) {
				...
				if(i == 5) {
					not_null = (std::string(p) == "AMERICA"); //for normal ssb
					// final_test_44 0.4 also for final_test_234 0.4
	...
	static void Part(std::string tbl_path, std::string binary_path){
		TupleSchema supplier_schema = SSB_Q4_1_Schema::part();
		Table supplier(supplier_schema, HeapBufferAllocator::Get());
		FILE* fp = fopen(tbl_path.c_str(),"r");
		char buffer[8192];
		rowid_t row_id = 0;
