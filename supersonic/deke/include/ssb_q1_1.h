...
public:
	...
	static void Lineorder(std::string tbl_path, std::string binary_path){
		...
		TupleSchema group_key = SSB_Q1_1_Schema::groupkey();
		...
		while(fgets(buffer, 8192, fp)) {
			...
			for(int i = 0; i < lineorder_schema.attribute_count() && p; i++) {
				if(i == 5 || i == 15) {
					...
					if(lineorder_schema.attribute(i).type() == INT64) {
						if(i == 11 && !(atoi(p) >= 1 && atoi(p) <= 3)) not_null = false;//lo_discount
						if(i == 8 && !(atoi(p) < 25)) not_null = false;//lo_quantity
			...
			lineorder.Set<INT64>(lineorder_schema.attribute_count()-2, row_id, row_id);
