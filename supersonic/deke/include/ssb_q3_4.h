#ifndef INCLUDE_SSB_Q3_4_H
#define INCLUDE_SSB_Q3_4_H
	...
	static void Customer(std::string tbl_path, std::string binary_path){
		...
		while(fgets(buffer, 8192, fp)) {
			...
			for(int i = 0; i < 8 && p; i++) {
				...
				if(i == 3) {
					not_null = (std::string(p) == "UNITED KI1" || std::string(p) == "UNITED KI5");
				}
				p = strtok(NULL, "|");
			}
