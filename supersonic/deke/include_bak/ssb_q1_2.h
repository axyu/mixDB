...
public:
	...
	static TupleSchema date() {
		TupleSchema date_schema;
		date_schema.add_attribute(Attribute("D_YEARMONTHNUM", INT64, NULLABLE));
		return date_schema;
	}
...
class Table2Data_Q1_2 {
public:
	static void Date(std::string tbl_path, std::string binary_path){
		TupleSchema date_schema = SSB_Q1_2_Schema::date();
	...
	static void Lineorder(std::string tbl_path, std::string binary_path){
		TupleSchema lineorder_schema = SSB_Q1_2_Schema::lineorder();
		TupleSchema group_key = SSB_Q1_2_Schema::groupkey();
			...
			for(int i = 0; i < lineorder_schema.attribute_count() && p; i++) {
				if(i == 5 || i == 15) {
					...
					if(lineorder_schema.attribute(i).type() == INT64) {
						if(i == 11 && !(atoi(p) >= 4 && atoi(p) <= 6)) not_null = false;//lo_discount
