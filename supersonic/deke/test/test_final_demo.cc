...
int main(int argc, char* argv[]) {
	...
	//}
	if( argc != 4) {
		cout << "./this row_group_count width run_time" << endl;
		exit(-1);
	}
	int rgc = atoi(argv[1]);
	int mw	= atoi(argv[2]);
	int run_time = atoi(argv[3]);
	cout << "rgc = " << rgc << " mw = " << mw << " run_time = " << run_time << endl;
	...
	vector<int> column_width = {40, 56, 24, 4};
...
*/
	...
	vector<vector<int>> classic_filter_order_q1(row_group_count, {0, 2, 1});
...
// classic query
	...
	int classic_query_time = get_time(begin, end);
...
//optimized query
	...
	int optimized_query_time = get_time(begin, end);

	vector<vector<rowcount_t>> optimized_visit_freq = optimized_query_fact_view.ColumnPieceVisitTimes(ag_gr_p);
	
	std::cout << "result : row_group_count = " << row_group_count
	<< " memory_width = " << mw
	<< " run_time = " << run_time
	<< " | " << classic_query_time << " " << optimized_query_time << std::endl;
