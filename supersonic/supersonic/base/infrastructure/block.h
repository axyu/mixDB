...
namespace supersonic {
	...
	class Column {
		public:
			...
			VariantConstPointer data_plus_offset_through_column_piece(rowid_t offset) const {
				CHECK_LE(offset / rowGroupSize, column_piece_deque_->size());
				return column_piece_deque_->at(offset / rowGroupSize)->data_plus_offset(offset % rowGroupSize);
			}

			// Fetch variable length data from the column piece.
			...
			VariantConstPointer data_plus_offset(rowid_t offset) const {
				...
				//if(offset / rowGroupSize < column_piece_deque_->size()){
				//return column_piece_deque_->at(offset / rowGroupSize)->data_plus_offset(offset % rowGroupSize);
				//}
			...
			bool RebuildColumnPieceVector(const rowcount_t rowcount){
				if(rowcount != -1) {
					...
					for(deque<shared_ptr<ColumnPiece>>::iterator it = column_piece_deque_->begin(); it != column_piece_deque_->end(); it++){
			...
			const deque<shared_ptr<ColumnPiece>>& column_piece_deque( ) const{
				return *column_piece_deque_;
			}
			...
			void ResetFrom(const Column& other) {
				...
				deque<shared_ptr<ColumnPiece>>* tmpvector = new deque<shared_ptr<ColumnPiece>>(other.column_piece_deque());
				column_piece_deque_->clear();
				delete column_piece_deque_;
				column_piece_deque_ = tmpvector;
			}

			// Updates the column to point to a new place, the same as pointed to by the
			// specified column.
			void ResetFromFront(const Column& other, rowcount_t row_count) {
				CheckInitialized();
				DCHECK_EQ(type_info().type(), other.type_info().type())
					<< "Type mismatch; trying to reset " << type_info().name() << " from "
					<< other.type_info().name();
				data_ = other.data();
				is_null_ = other.is_null();
				int add = ((row_count % rowGroupSize) != 0) ? 1 : 0;
				deque<shared_ptr<ColumnPiece>>* tmpvector = new deque<shared_ptr<ColumnPiece>>(other.column_piece_deque().begin(), other.column_piece_deque().begin() + row_count / rowGroupSize + add);
				column_piece_deque_->clear();
				delete column_piece_deque_;
				column_piece_deque_ = tmpvector;
			}
			...
			void ResetFromPlusOffset(const Column& other, const rowcount_t offset) {
				...
				if(this != &other) {
					deque<shared_ptr<ColumnPiece>>* tmpvector = new deque<shared_ptr<ColumnPiece>>();
					const deque<shared_ptr<ColumnPiece>>& other_tmp_vector = other.column_piece_deque();
					...
					for(int i = 0; i < offset / rowGroupSize; i++) {
						column_piece_deque_->pop_front();
					}
				}
			}
			...
			VariantConstPointer PrepareData(rowcount_t offset) const {
				...
				column_piece_deque_->at(offset / rowGroupSize)->PrepareData();
				return column_piece_deque_->at(offset / rowGroupSize)->data();
			}

			// Return the column piece according to an offset
			const ColumnPiece& column_piece(rowcount_t offset) const {
				DCHECK_LE(offset / rowGroupSize, column_piece_deque_->size() )
					<< "in column_piece offset out of column_piece_deque_.size() ";
				return *(column_piece_deque_->at(offset / rowGroupSize));
			}
			...
			void AppendColumnPiece(const ColumnPiece& column_piece,
				...
				if(column_piece_deque_->size() <= offset / rowGroupSize) {
					column_piece_deque_->push_back(shared_ptr<ColumnPiece>(
								new ColumnPiece(column_piece, data_in_memory, in_memory_offset, offset, storage_type, type_info)));
				} else {
					column_piece_deque_->erase(column_piece_deque_->begin() + offset / rowGroupSize);
					column_piece_deque_->insert(column_piece_deque_->begin() + offset / rowGroupSize,
			...
			void ResetVisitTimes() {
				for(int i = 0; i < column_piece_deque_->size(); i++) {
					column_piece_deque_->at(i)->ResetVisitTimes();
				}
			}
			...
			void PrintColumnPiecesInfo(rowcount_t row_count) const {
				...
				for(rowcount_t i = 0; i < row_count; i += rowGroupSize) {
					if(type_info().type() == INT32){
						PrintColumnPiece<INT32>(*(column_piece_deque_->at(i / rowGroupSize)));
					}else if(type_info().type() == DOUBLE){
						PrintColumnPiece<DOUBLE>(*(column_piece_deque_->at(i / rowGroupSize)));
					}else if(type_info().type() == INT64){
						PrintColumnPiece<INT64>(*(column_piece_deque_->at(i / rowGroupSize)));
					}else if(type_info().type() == STRING){
						PrintColumnPiece<STRING>(*(column_piece_deque_->at(i / rowGroupSize)));
					}
				}
			}
		...
		private:
			...
			friend class View;
			Column() : attribute_(NULL), type_info_(NULL), data_(NULL), is_null_(NULL), column_piece_deque_(NULL) {}

			// Clear the column piece vector.
			void ClearColumnPieceVector() {
				if(column_piece_deque_ != NULL) {
					column_piece_deque_->clear();
				}
			}
			...
			void Init(const Attribute* attribute) {
				...
				column_piece_deque_ = new deque<shared_ptr<ColumnPiece>>();
			}

			void CheckInitialized() const {
				DCHECK(type_info_ != NULL) << "Column not initialized";
				DCHECK(column_piece_deque_ != NULL) << "Column Piece vector not initialized";
			}
			...
			deque<shared_ptr<ColumnPiece>>* column_piece_deque_;
	...
	class View {
		public:
			...
			explicit View(const View& other, rowcount_t row_count)
				: schema_(other.schema()),
				columns_(new Column[other.schema().attribute_count()]),
				row_count_(0) {
					Init();
					ResetFromFront(other, row_count);
				}
			...
			// Resets View's columns from another View's Front. Sets the row_count as well.
			void ResetFromFront(const View& other, rowcount_t row_count) {
				for (int i = 0; i < schema_.attribute_count(); ++i) {
					mutable_column(i)->ResetFromFront(other.column(i), row_count);
				}
			...
			void ResetVisitTimes() {
				for (int i = 0; i < column_count(); i++) {
					mutable_column(i)->ResetVisitTimes();
				}	
			}
		private:
			void Init() {
