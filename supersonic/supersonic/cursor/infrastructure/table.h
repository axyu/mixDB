...
class Table : public BasicOperation {
 public:
  ...
  bool SetRowCapacity(rowcount_t row_capacity,vector<rowcount_t> row_capacity_vector);
  ...
  //bool ReserveRowCapacity(rowcount_t needed_capacity,vector<rowcount_t> needed_capacity_vector);
  ...
  // OOM occurs.
  rowcount_t AppendView(const View& view,const vector<StorageType> storage_type_vector = vector<StorageType>(50, MEMORY));
