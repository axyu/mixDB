...
bool Table::SetRowCapacity(rowcount_t row_capacity,vector<rowcount_t> row_capacity_vector) {
	  CHECK_GE(row_capacity, view_.row_count())
	      << "New table capacity must be equal to or exceed current row count";
...
bool Table::ReserveRowCapacityOneTime(rowcount_t total_row_capacity, const vector<rowcount_t>& in_memory_row_capacity) {
...
rowcount_t Table::AppendView(const View& view, const vector<StorageType> storage_type_vector) {
