...
namespace supersonic {
	...
	class AggregationSpecification {
		public:
			...
			static AggregationSpecification& CreateSameOutputNameAS(const AggregationSpecification& aggregation_specification) {
				AggregationSpecification* result = new AggregationSpecification();
				for(int index = 0; index < aggregation_specification.size(); index++) {
					if(!aggregation_specification.aggregation(index).output_type_specified()) {
						result->add(
								Element(aggregation_specification.aggregation(index).aggregation_operator(),
									aggregation_specification.aggregation(index).output(),
									aggregation_specification.aggregation(index).output(),
									aggregation_specification.aggregation(index).is_distinct()));
					} else {
						result->add(
								Element(aggregation_specification.aggregation(index).aggregation_operator(),
									aggregation_specification.aggregation(index).output(),
									aggregation_specification.aggregation(index).output(),
									aggregation_specification.aggregation(index).output_type(),
									aggregation_specification.aggregation(index).is_distinct()));
					}
				}
				return *result;
	...
	FailureOrOwned<Cursor>
		BoundGroupAggregate(
				...
				bool best_effort,
				const vector<vector<int>>* filter_order,
				vector<const View*> encoded_group_vectors,
				Cursor* child);
