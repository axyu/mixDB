...
inline FailureOrVoid UpdateSelectionAggregation(
    int input_position, aggregations::ColumnAggregator* column_aggregator,
    const View& view, const rowid_t result_index_map[], const vector<rowid_t>& selected_inputs_indexes) {
  const Column* input;
  if (input_position != -1) {
    input = &view.column(input_position);
  } else {
  ...
  return column_aggregator->UpdateAggregationForSelectedInputs(
      input, view.row_count(), result_index_map, selected_inputs_indexes);
}
...
FailureOrVoid Aggregator::UpdateSelectionAggregations(const View& view,
                                             const rowid_t result_index_map[],
  ...
  vector<pair<int, linked_ptr<aggregations::ColumnAggregator> > >::iterator it;
  for (it = column_aggregator_.begin(); it != column_aggregator_.end(); ++it) {
    PROPAGATE_ON_FAILURE(UpdateSelectionAggregation(it->first,
                                           it->second.get(),
                                           view,
                                           result_index_map,
